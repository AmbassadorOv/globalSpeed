...
export function SpeedControl(props: SpeedControlProps) {
  ...
  return <div className="SpeedControl">
    ...
    props.onChange(clamp(MIN_SPEED_CHROMIUM, MAX_SPEED_CHROMIUM, props.speed + speedDelta))
      ...
      <button aria-label={gvar.gsm.token.decreaseSpeedLarge} title={gvar.gsm.token.decreaseSpeedLarge} onClick={() =>  handleAddDelta(-largeStep)}><FaAngleDoubleLeft size={"1.14rem"}/></button>
      <button aria-label={gvar.gsm.token.decreaseSpeedSmall} title={gvar.gsm.token.decreaseSpeedSmall} onClick={() =>  handleAddDelta(-smallStep)}><FaAngleLeft size={"1.14rem"}/></button>
      <NumericInput rounding={2} noNull={true} min={MIN_SPEED_CHROMIUM} max={MAX_SPEED_CHROMIUM} value={props.speed} onChange={v => {
        props.onChange(v)
      }}/>
      <button aria-label={gvar.gsm.token.increaseSpeedSmall} title={gvar.gsm.token.increaseSpeedSmall} onClick={() =>  handleAddDelta(smallStep)}><FaAngleRight size={"1.14rem"}/></button>
      <button aria-label={gvar.gsm.token.increaseSpeedLarge} title={gvar.gsm.token.increaseSpeedLarge} onMouseDown={() => {}} onClick={() =>  handleAddDelta(largeStep)}><FaAngleDoubleRight size={"1.14rem"}/></button>
    </div>
