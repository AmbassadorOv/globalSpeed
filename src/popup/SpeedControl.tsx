...
export function SpeedControl(props: SpeedControlProps) {
  ...
  return <div className="SpeedControl">
    ...
    props.onChange(clamp(MIN_SPEED_CHROMIUM, MAX_SPEED_CHROMIUM, props.speed + speedDelta))
      ...
      <button aria-label="Decrease speed by a large step" title="Decrease speed by a large step" onClick={() =>  handleAddDelta(-largeStep)}><FaAngleDoubleLeft size={"1.14rem"}/></button>
      <button aria-label="Decrease speed by a small step" title="Decrease speed by a small step" onClick={() =>  handleAddDelta(-smallStep)}><FaAngleLeft size={"1.14rem"}/></button>
      <NumericInput rounding={2} noNull={true} min={MIN_SPEED_CHROMIUM} max={MAX_SPEED_CHROMIUM} value={props.speed} onChange={v => {
        props.onChange(v)
      }}/>
      <button aria-label="Increase speed by a small step" title="Increase speed by a small step" onClick={() =>  handleAddDelta(smallStep)}><FaAngleRight size={"1.14rem"}/></button>
      <button aria-label="Increase speed by a large step" title="Increase speed by a large step" onMouseDown={() => {}} onClick={() =>  handleAddDelta(largeStep)}><FaAngleDoubleRight size={"1.14rem"}/></button>
    </div>
