...
export function SpeedControl(props: SpeedControlProps) {
  ...
  return <div className="SpeedControl">
    ...
    props.onChange(clamp(MIN_SPEED_CHROMIUM, MAX_SPEED_CHROMIUM, props.speed + speedDelta))
      ...
      <button aria-label={gvar.gsm.token.decrease} title={gvar.gsm.token.decrease} onClick={() =>  handleAddDelta(-largeStep)}><FaAngleDoubleLeft size={"1.14rem"}/></button>
      <button aria-label={gvar.gsm.token.decrease} title={gvar.gsm.token.decrease} onClick={() =>  handleAddDelta(-smallStep)}><FaAngleLeft size={"1.14rem"}/></button>
      <NumericInput rounding={2} noNull={true} min={MIN_SPEED_CHROMIUM} max={MAX_SPEED_CHROMIUM} value={props.speed} onChange={v => {
        props.onChange(v)
      }}/>
      <button aria-label={gvar.gsm.token.increase} title={gvar.gsm.token.increase} onClick={() =>  handleAddDelta(smallStep)}><FaAngleRight size={"1.14rem"}/></button>
      <button aria-label={gvar.gsm.token.increase} title={gvar.gsm.token.increase} onMouseDown={() => {}} onClick={() =>  handleAddDelta(largeStep)}><FaAngleDoubleRight size={"1.14rem"}/></button>
    </div>
    ...
    {!!view.speedSlider && (
      <div className="slider">
        <button
          ...
          className={`icon-button ${view.freePitch ? "active" : ""}`}
          onClick={(e) => {
            if (!view.freePitch) {
